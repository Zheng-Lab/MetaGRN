...
public class RandomForest {
	...
	private double[][] finalNetwork;	/* the final result*/
	 ...
	 * @throws IOException
	 */
	public RandomForest(ArrayList<ArrayList<Double>> exprMatrix, ArrayList<Integer> inputIdx, String treeMethod, String K, int nbTrees) throws IOException{
		this.params = new Genie3TimeParam(exprMatrix, inputIdx, treeMethod, "sqrt"/* use the default value, don't change*/, nbTrees);
		
		int nbGenes = exprMatrix.get(0).size();					/* the number of genes*/
		finalNetwork = new double[nbGenes][nbGenes];			/* final network*/
	}
	...
	public void run() {
		...
		int nbGenes = exprMatrix.get(0).size();					/* the number of genes*/
		int numTimePoints = exprMatrix.size();					/* the number of time points*/
...
//		}
	...
	public void oneIteration(int nargin, ArrayList<ArrayList<Double>> exprMatrix, int nbGenes, int numTimePoints, int geneIndex) {
		...
		if(nargin == 1) {
			singleParams = new Genie3SingleTimeParam(exprMatrix, geneIndex);
		} else if(nargin == 2) {
			singleParams = new Genie3SingleTimeParam(exprMatrix, geneIndex, params.getInputIdx());
		} else if(nargin == 3) {
			singleParams = new Genie3SingleTimeParam(exprMatrix, geneIndex, params.getInputIdx(), params.getTreeMethod());
		} else if(nargin == 4) {
			singleParams = new Genie3SingleTimeParam(exprMatrix, geneIndex, params.getInputIdx(), params.getTreeMethod(), params.getK());
		} else if(nargin == 5) {
			singleParams = new Genie3SingleTimeParam(exprMatrix, geneIndex, params.getInputIdx(), params.getTreeMethod(), params.getK(), params.getNbTrees());
		}
		...
		for(int k = 1; k<= regSize; k++)
		{
			...
			finalNetwork[regulator-1][geneIndex-1] = 1;
		}
	}
	
	public Genie3TimeParam getParams() {
		return params;
	}
	
	public double[][] getFinalNetwork() {
		return finalNetwork;
	}
}
