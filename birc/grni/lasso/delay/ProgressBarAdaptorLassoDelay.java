...
public class ProgressBarAdaptorLassoDelay extends SwingWorker<Void, Void>{
	...
	//LIU
	private InputData inputDataObj = null;			/* the InputData object contains more information of input data file format, such as with or without header, headers of rows and columns*/
	
	//LIU
//	public ProgressBarAdaptorLassoDelay (String path, int delayMax){
//		/* Read input data */
...
//	}
	...
	public ProgressBarAdaptorLassoDelay (InputData inputDataObj, int delayMax) {
		this.inputDataObj = inputDataObj;
		this.maxDelay = delayMax;
		
		ArrayList<ArrayList<Double>> inputDataArrayList = this.inputDataObj.getData();
		this.numGenes = inputDataArrayList.get(0).size();
		this.numSamples = inputDataArrayList.size();
		double[][] inputDataArray = new double[this.numSamples][this.numGenes];
		/* transform input data matrix from the two-dimension Double ArrayList to a two-dimension double array*/
		for(int rowOfInputDataArrayList = 0; rowOfInputDataArrayList< inputDataArrayList.size(); rowOfInputDataArrayList++) {
			for(int columnOfInputDataArrayList = 0; columnOfInputDataArrayList< inputDataArrayList.get(rowOfInputDataArrayList).size(); columnOfInputDataArrayList++) {
				inputDataArray[rowOfInputDataArrayList][columnOfInputDataArrayList] = inputDataArrayList.get(rowOfInputDataArrayList).get(columnOfInputDataArrayList);
			}
		}
		
		/* data normalization */
		this.inputData =  birc.grni.util.CommonUtil.zeroMean(inputDataArray);
	...
	protected void done() {
		...
		//LIU: add gene names information
//		GrnTimeDelayLasso.timeDelayLassoResult(transposeOfFinalNetworkdd, transposeOfDelayMatrixdd, numGenes);
		GrnTimeDelayLasso.timeDelayLassoResult(transposeOfFinalNetworkdd, transposeOfDelayMatrixdd, numGenes, this.inputDataObj.getGeneNames());
	}
