...
public class Binomial_HDBNprof  
...
// new constructor
public Binomial_HDBNprof( double[] var, int ord, PRNG prng, int itr)
{
nogenes = (int)Math.sqrt(var.length);
maxitr = itr;
//maxpcnt = nogenes;
...
//if(mi[node][parent-nogenes/2][0]>=th && parents.length < 12)  			
 ...
 private void reverseOrRemoveEdge(int node, int parent)
	...
	{
		double score = Binomial_HDBNmlf.getNodePotential(i,dagglob);
		score += Binomial_HDBNmlf.getNodePotential(j,dagglob);
