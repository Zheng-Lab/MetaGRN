...
public class Binomial_HDBNmlf implements UnnormalizedDensity
{
  static int itrno =0; 
  ...
  static Table tuples;
  private static Logger logger =Logger.getLogger(Binomial_HDBNmlf.class.getName());
  ...
  double bic() {
    //Binomial_HDBNf.iteration++;
	  ProgressBarAdaptorDBN.iteration++;
    // ouput graph     
    double ans = 0;
  ...
  try{  
    ...
    if(ProgressBarAdaptorDBN.iteration > itrno){
    ...
    if(ProgressBarAdaptorDBN.iteration == (itrno*2 - 1)){
    	   for(int i=0; i<nogenes; i++){
 	       ...
 	       for(m=0; m<nogenes/(order+1); m++){
    		   for( n=0; n<nogenes/(order+1); n++){
    			   ...
    			   if(avgdag[m*nogenes + (n+(nogenes/(order+1)))] >= 0.5){
    				   ...
    				   ProgressBarAdaptorDBN.finalNetwork[m*actualgenes + n]=1;
  ...
  public static double bic2(int[][] dag){
     ...
     for(int i=0; i<nogenes; i++){
      ...
      if(ProgressBarAdaptorDBN.iteration%2==0)convg[ProgressBarAdaptorDBN.iteration/2-1]=ans;
  ...
  public Binomial_HDBNmlf(int n, int ord,  int itr, int[][] inputData) 
  {
   nogenes = n; order = ord;  
   network = new int[nogenes*nogenes];
   avgdag = new double[nogenes*nogenes];
  ...
  public Binomial_HDBNmlf(int n, int ord,  int itr, int[][] inputData, int betaValue, String priorFile) 
  {
