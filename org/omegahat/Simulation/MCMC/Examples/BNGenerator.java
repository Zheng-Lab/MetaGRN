//================================================================
//     Copyright (c) 2003, Escola Politécnica-USP
//                     All Rights Reserved
//================================================================
//
// NAME : BNGenerator
...
public class BNGenerator {
    ...
    public BNGenerator(int nVertices, int maxDeg) {
        ...
        dagnew = new int[nVertices][nVertices];
    }
    /**
    ...
    public static void main(String argv[]) throws Exception {
        ...
        if	((structure.compareTo("multi") == 0)&&(maxInducedWidth==-1)) {	// multi-connected graph is the default structure
            bn.generateMultiConnected(nGraphs,nIterations,maxValues,testU,format,baseFileName,dagnew);
     ...
     *	CALL 
     **/
    public int[][] call(String argv[], int[][] dag, int ittype) throws Exception {
        int nGraphs=1;
        //int totalDegree=10;
        String testU="no"; // default 	// for testing Uniformity
        String structure="multi"; // default structure
        ...
        /*for (int i=0;i<dag[1].length;i++)
          for (int j=0;j<dag[1].length;j++)
...
//        bn.inicializeGraph(); // Inicialize a simple ordered tree as a BN structure
         if(ittype>2)bn.inicializeGraphm(dag); // Inicialize a simple ordered tree as a BN structure
        else bn.inicializeGraph();
        ...
        if	((structure.compareTo("multi") == 0)&&(maxInducedWidth==-1)) {	// multi-connected graph is the default structure
            dagnew = bn.generateMultiConnected(nGraphs,nIterations,maxValues,testU,format,baseFileName,dag);
         
        /*  for(int i=0;i<dagnew[1].length;i++)
            for(int j=0;j<dagnew[1].length;j++)
                System.out.print(dagnew[i][j]); */
        ...
        for (int g = 0; g < nGraphs; g++) {
            ...
            if ( (testU.compareTo("yes") == 0))	{
                ...
                if ((format.compareTo("xml") ==0))	{
                    saveBNXML2(baseFileName,format,g,maxValues);
                }
     ...
     */
    int[][] generateMultiConnected(int nGraphs, int nIterations, int maxValues,
    String testU, String format, String baseFileName, int[][] dag)	throws Exception	{
        int inducedWidth=1;
        int totalArcs=nNodes-1; // Simple tree has (nNodes-1)arcs
        for (int g = 0; g < nGraphs; g++) {
            //inicializeGraphm(dag);	// Inicialize a simple ordered tree as a BN structure
 ...
 */
  public int getInducedWidth(){ return 3; }  
...
/*
  public int getInducedWidth() {
    ...
    public void loadToEmBayes(String name){ }    
    /*
    ...
    */
    ...
    public void inicializeGraphi(){              // initialize a simple ordered tree
       ...
       /* for (int i=0;i<nNodes;i++ ) {
         ...
         int[][] dag = new int[nNodes][nNodes];
         try{
            FileReader fr = new FileReader("dagbng.xml");
            ...
            do
            {
                ...
                for(int i=0; i<nNodes; i++)
                {
                        dag[j][i]=Integer.parseInt(cell[i]);
                }
                j++;
            }
         ...
         for(int i=0;i<nNodes;i++)
         {
          ...
          int pcnt = 1;
          for(int j=0;j<nNodes;j++)        
            if(dag[i][j]>0){parentMatrix[i][pcnt]=j; pcnt++;}
         } 
         ...
         { 
          ...
          int pcnt = 1;
          for(int j=0;j<nNodes;j++)
            if(dag[j][i]>0){sonMatrix[i][pcnt]=j; pcnt++;}
         }

    }

    public void inicializeGraphm(int[][] dag){              // initialize a simple ordered tree
...
*/
    ...
    private void saveBNXML2(String name,String format,int n, int maxValues) throws Exception { // Save BN in XML format
        
        /*  dagnew = new int[nNodes][nNodes];
         for(int i=0;i<nNodes;i++)
          for(int j=0;j<nNodes;j++)
            dagnew[i][j]=0;*/
        try{
         ...
         {
          for(int j=1;j<maxDegree+1;j++)        
            if(parentMatrix[i][j]>-1)dagnew[i][parentMatrix[i][j]]=1;
         } 
        }
        catch(Exception e){System.out.println("got it ");}
        /* for(int i=0;i<nNodes;i++)
          for(int j=0;j<nNodes;j++)
            System.out.print(dagnew[i][j]);
    ...
    void averageQuiSquare(int nGra,int nIterati,int maxVal,String te,String fo,String baseFileNam,int maxInducedW) throws Exception{
        ...
        for (int i=0;i<nSet;i++)    {
            ...
            inicializeGraphi(); // Inicialize a simple ordered tree as a BN structure
