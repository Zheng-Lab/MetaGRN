...
public class NodePropertiesPanel extends JPanel implements ActionListener, DocumentListener, NodeManagerListener {
    ...
    protected void init() {
        ...
        // Label & Value panel
        //
        labelValuePanel = new JPanel(); labelValuePanel.setLayout(new GridBagLayout());
        labelValueTab = new JPanel(); labelValueTab.setLayout(new GridBagLayout());
        labelValueTab.setBorder(new EtchedBorder());
        tabPane.addTab("Values",labelValueTab);

        nameField = new JTextField(30); nameFieldDocument = nameField.getDocument();
        nameFieldDocument.addDocumentListener(this);
        labelField = new JTextField(30); labelFieldDocument = labelField.getDocument();
        labelFieldDocument.addDocumentListener(this);
        addStateField = new JTextField(16); addStateField.addActionListener(this);
        stateList = new JList(); stateList.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
        stateListModel = new DefaultListModel();
        stateList.setModel(stateListModel);
        evidenceCombo = new JComboBox(); evidenceCombo.addActionListener(this);
        JScrollPane stateScroll = new JScrollPane(stateList);
        Dimension dim = new Dimension(140,80);
        stateScroll.setPreferredSize(dim); stateScroll.setMaximumSize(dim);
        dim = new Dimension(140,20);
        addStateField.setPreferredSize(dim); addStateField.setMaximumSize(dim);
        dim = new Dimension(80,20);
        addStateButton = new JButton("Add"); addStateButton.addActionListener(this);
        addStateButton.setPreferredSize(dim); addStateButton.setMaximumSize(dim);
        removeStateButton = new JButton("Remove"); removeStateButton.addActionListener(this);
        removeStateButton.setPreferredSize(dim); removeStateButton.setMaximumSize(dim);
        dim = new Dimension(140,20);
        evidenceCombo.setPreferredSize(dim); evidenceCombo.setMaximumSize(dim);
        ...
        // CPT panel
        //
        cpfPanel = new JPanel(); cpfPanel.setLayout(new GridBagLayout());
        cpfTab = new JPanel(); cpfTab.setLayout(new GridBagLayout());
    ...
    public void setNode(VisualVertex node) {
        ...
        nameField.setText(bbnNode.getName());
        ignoreEvent = false;
    ...
    protected void prepareCPFDisplay() {
        ...
        if (headerRows > 0) {
            ...
            for (Iterator i = eligibleParents.iterator(); i.hasNext(); index--) {
                BBNNode parent = (BBNNode) i.next();
                BBNDiscreteValue parentValue = (BBNDiscreteValue) parent.getValues();
                JLabel label = new JLabel(parent.getName());
                label.setBorder(new EtchedBorder());
                GUIUtil.gbAdd(cpfPanel, label, 0, index, 1,1, 0,0,0,0, GridBagConstraints.BOTH, GridBagConstraints.WEST, 1.0,1.0);
    ...
    protected void layoutCPTFields(LinkedList parents, LinkedList widths, int column, int row, Hashtable query, BBNCPF cpf) {
        ...
        for (Iterator i = parentValue.iterator(); i.hasNext(); ) {
            Object value = i.next();
            query.put(parent.getName(), value);
            if (isLast) {
                CPFEntry entry = new CPFEntry(cpf, query);
                entry.setEditable(graphEditable);
