 ...
 */
public class LikelihoodWeighting extends MCMC {
    ...
    public LikelihoodWeighting(BBNGraph g, int i) {
        super(g);
        setMaxIteration(i);
    }
	 ...
	 * 
	 * @return String - the name of the inference algorithm (LikelihoodWeighting)
	 */
	...
	public InferenceResult getMarginals() {
        ...
        for (int iteration = 0; (!abort && (iteration < maxIteration)); iteration++) {
        ...
        if (rmseWriter != null) {  // Bug fix courtesy of Anshu Sakseena
            rmseWriter.dump();
            rmseWriter.close();
        }
		return result;
	...
	public static void main(String[] args) {
        ...
        if (inputFile == null) {
            System.out.println("Usage: edu.ksu.cis.bnj.bbn.inference.approximate.sampling.LikelihoodWeighting -i:inputfile [-e:evidencefile] [-o:outputfile] [-q] [-n:numiteration] [-l:runTimeLimit] [-r] [-ro:rmseOutputFile] [-m] [-d:generateddataoutputfile]"); // XXX
        ...
        LikelihoodWeighting lw = new LikelihoodWeighting(g);
        lw.setMaxIteration(numIter);
        lw.setUseMarkovBlanketScore(markov);
        lw.generateData(dataOutput != null);
        lw.setRunningTimeLimit(runningTimeLimit); // XXX
        
		  if (rmseOut != null) {
				try {
                    FileOutputStream outfile = new FileOutputStream(rmseOut);
					lw.setRMSEfile(outfile);
				}
        ...
        InferenceResult result = lw.getMarginals();

        freemem = origfreemem - r.freeMemory();
        long learnTime = System.currentTimeMillis();

        if (outputFile != null) result.save(outputFile);
        if (dataOutput != null) lw.tuples.save(dataOutput);

        if (!quiet) {
            System.out.println("Final result:");
            System.out.println(result.toString());
            System.out.println("Memory needed for Likelihood Weighting = "+freemem);
