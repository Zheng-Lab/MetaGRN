...
import edu.ksu.cis.bnj.bbn.datagen.*;
        ...
        for (int i = 0; i < max; i++) {
            ...
            if (node.isEvidence()) {
                ...
                for (Iterator j = parents.iterator(); j.hasNext(); ) {
                    ...
                    Integer parentIndex = (Integer) indexCache.get(parent.getName());
            ...
            if ((iteration+1) % updateIteration == 0) {
                ...
                for (int i = 0; i < max; i++) {
                    //if (isUpdated.contains(new Integer(i))) {
                        ...
                        newTotalTable.normalizeAndFill(nodes[i].getName());
                        //totalTable[i].normalizeAndFill(nodes[i].getName());
                        icpt[i] = (BBNCPF) origCPT[i].clone(); // this is expensive
                        icpt[i].multiply(r);
                        //icpt[i].add(totalTable[i]);
                        icpt[i].add(newTotalTable);
                        icpt[i].normalize(nodes[i].getName());
                    //}
                }
            }
