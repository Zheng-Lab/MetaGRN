 ...
 */
public class BICScore extends LearnerScore {
	...
	public double getScore(int curNode, int candidate, Set[] parentTable) {
        ...
        if (arraySize > 0) {
        	...
        	for (int j = 0; j < nodeArity; j++) { 
                ...
                if(n_ijk>0){result += n_ijk*Math.log((n_ijk*1.0)/tupleSize);param++;}
            }
           
        }

        result -= 0.5*param*Math.log(tupleSize);
        return result;
	}
            ...
            if (depth == nodes.length-1) {
                ...
                if (n_ij > 0) {
                    for (int j = 0; j < nodeArity; j++) { 
                        ...
                        //pi_n_ijk+=Math.log(n_ijk);
                       if(n_ijk>0){result += n_ijk*Math.log((n_ijk*1.0)/n_ij);param++;}
                       //System.out.println(n_ijk+" "+result);
                    }
                }
