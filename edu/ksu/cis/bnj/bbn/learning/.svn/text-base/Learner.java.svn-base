...
public abstract class Learner implements Optionable {
    ...
    protected BBNGraph populateNodes() {
        ...
        for (int i = 0; i < max; i++) {
            ...
            bbnNodes[i] = node;
            indexTable.put(node.getName(), new Integer(i));
            graph.add(node);
        }
    ...
    public static Learner load(String className, Data t) {
        try {
            Learner l = (Learner) FileClassLoader.loadAndInstantiate(className, learnerClassName, null,
                new Class[] { Data.class }, new Object[] {t});
            return l; 
        } catch (Exception e) {
            e.printStackTrace();
        }
