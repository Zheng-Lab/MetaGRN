...
import edu.ksu.cis.bnj.bbn.inference.ls.*;
...
{	
	...
	public Table generateData(int noOfSamples)
	{
        ...
        for (Iterator i = mNetworkNodes.iterator(); i.hasNext(); nodeIndex++) {
            ...
            Attribute attribute = new Attribute(node.getName());
	...
	private void setEvidenceToRootNodes (BBNNode node, int nodeIndex, Hashtable cpfValues,  
									double[] cummulativeCPT, BBNDiscreteValue stateValues)
	{
			int probIndex = 0;
	...
	private void setEvidenceToNodes(BBNNode node, int nodeIndex,  
									double[] cummulativeCPT, BBNDiscreteValue stateValues)
	{
			double randomValue = Settings.random.nextDouble();
	...
	private void setEvidenceValue(BBNNode node, int nodeIndex, double randomValue, BBNDiscreteValue stateValues,  
								double[] cummulativeCPT)
	{
		int probIndex = 0;
		...
		{	
			if (randomValue <= cummulativeCPT[probIndex])
			{
                    ...
                    nodeName2Value.put(node.getName(), stateValue);
