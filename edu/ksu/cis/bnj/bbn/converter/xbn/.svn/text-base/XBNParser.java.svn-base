	...
	public void save(OutputStream stream, BBNGraph graph) {
        ...
        try {
            ...
            for (Iterator i = nodes.iterator(); i.hasNext(); ) {
                ...
                String header = "  <ARC PARENT=\""+mangleXMLString(node.getName())+"\" CHILD=\""; //$NON-NLS-1$ //$NON-NLS-2$
                for (Iterator j = children.iterator(); j.hasNext(); ) {
                    BBNNode child = (BBNNode) j.next();
                    w.write(header+mangleXMLString(child.getName())+"\" />"+ln); //$NON-NLS-1$
                }
            }
            w.write("</STRUCTURE>"+ln); //$NON-NLS-1$
            ...
            for (Iterator i = nodes.iterator(); i.hasNext(); ) {
                ...
                if (parents == null || parents.size() == 0) {
                    ...
                    for (Iterator j = parents.iterator(); j.hasNext(); ) {
                        ...
                        w.write("      <CONDELEM NAME=\""+mangleXMLString(parent.getName())+"\" />"+ln); //$NON-NLS-1$ //$NON-NLS-2$
                    }
                    w.write("    </CONDSET>"+ln); //$NON-NLS-1$
                }
        ...
        for (int i = 0; i < max; i++) {
            ...
            switch(node.getNodeType()) {
                case Node.ELEMENT_NODE:
                    ...
                    } else throw new RuntimeException("Unhandled element "+name);
                    break;
