...
public class NetParser implements/*@bgen(jjtree)*/ NetParserTreeConstants,Converter, NetParserConstants {/*@bgen(jjtree)*/
  protected JJTNetParserState jjtree = new JJTNetParserState();protected Hashtable nodeCache = new Hashtable();
    ...
    public void save(OutputStream os, BBNGraph graph)
    {
        ...
        try {
            switch (saveFormat) {
                case FORMATv6_0:
                    ...
                    if (os instanceof ConverterOutputStream) { // This bug fix is courtesy of Jan-Diederik van Wees
                        ...
                        else graphName = graph.getName();
                    }
                    stream.write("class "+graphName+" {  % you may need to change this network name to match the filename"+ln); break; // $NON-NLS-1$ // $NON-NLS-2$
                case FORMATv5_7:
                    stream.write("net {"+ln); break; // $NON-NLS-1$
                default:
            ...
            switch (saveFormat) {
                case FORMATv6_0:
                     stream.write(ln); break;
                case FORMATv5_7:
                     stream.write("}"+ln+ln); filler = 0; break; // $NON-NLS-1$
                default:
                    throw new RuntimeException("Unknown save format value!");
            }
            ...
            for (Iterator i = nodes.iterator(); i.hasNext(); )
            {
                ...
                if (nodeNameList.size() > 0 && !node.isDecision())
                {
                    ...
                    String cpfdata = saveCPT(valueListTable, (Hashtable) cpfTable.get(nodeName),
                        nodeNameList, new Hashtable(), new StringBuffer(), tab+"        "); // $NON-NLS-1$
...
*/
  ...
  final public void Node() throws ParseException {
    ...
    t = jj_consume_token(IDENTIFIER);
    jj_consume_token(LBRACE);
                           name = t.image; node.setName(name);
    label_4:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case LABEL:
    ...
    while (true) {
      ...
      case MINUS:
        ...
        number = NumberLiteral();
              list.add(new Double(sgn*number.doubleValue())); sgn = 1;
        break;
      case NORMAL:
