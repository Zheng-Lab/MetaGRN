...
//		mNoOfNodes = mJDBCTally.getAttributeIndices().length;
   ...
   private int getAttributeIndexFromDataBase(String string)
   {
    ...
    public void displayTable(LinkedList[] listArray)
    {
        ...
        {   
            Attribute attr = (Attribute) mJDBCTally.getUnderlyingData().getAttributes().get(mAttributeIndices[i]);   
            //System.out.println(i + " "+  parentList[i]+ "<---- " +  attr.getName());           
        }
    }
	...
	{	
		...
		{	
			...
			mIndexTable.put(node.getName(), new Integer(j));
			bbnNodes[j] = node;
			graph.add(node);
		}
		return graph;
 ...
 /*   protected void computeCPT(BBNGraph g) 
	...
	public static void main(String[] args) 
    {
		...
		if (driver == null || url == null || login == null || passwd == null || 
			(inputFile != null && outputFile != null)|| (tableListString == null)) 
		{
			...
			System.out.println();
			System.out.println("NOTE: In some databases, table names are CASE SENSITIVE. "+
			"Some even behave strangely, like Oracle, who treats table names as case insensitive, but when "+
			"it comes to handling metadata, table names are case sensitive.");
			return;
		}
