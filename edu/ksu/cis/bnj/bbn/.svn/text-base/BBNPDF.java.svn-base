...
public class BBNPDF implements Cloneable {
    ...
    {
        return (o instanceof BBNContinuousValue) &&
            (((BBNContinuousValue) o).mean == mean) &&
            (((BBNContinuousValue) o).variance == variance) &&
            (((BBNContinuousValue) o).type == type);
    }
    ...
    public String toString() {
        switch (type) {
            case UNIFORM: return "uniform ("+getLowerBound()+", "+getUpperBound()+")"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            case NORMAL: return "normal ("+mean+", "+variance+")"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            case POISSON: return "poisson ("+mean+", "+variance+")"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            case CONSTANT: return String.valueOf(mean);
        }
