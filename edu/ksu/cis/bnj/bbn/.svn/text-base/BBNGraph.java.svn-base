...
import edu.ksu.cis.kdd.util.graph.*;
...
public class BBNGraph extends Graph {
    ...
    public Hashtable getEvidenceTable() {
        ...
        {
            BBNNode n = (BBNNode) i.next();
            if (n.isEvidence()) tbl.put(n.getName(), n.getEvidenceValue());
        }
    ...
    public void saveEvidence(String filename) {
        ...
        for (Iterator i = getNodes().iterator(); i.hasNext(); )
        {
            BBNNode n = (BBNNode) i.next();
            if (n.isEvidence())
                tbl.put(n.getName(), n.getEvidenceValue().toString());
        }
        try {
            if (tbl.size() > 0)
    ...
    public Object clone() {
        ...
        for (Iterator i = nodes.iterator(); i.hasNext(); ) {
            ...
            nodeTable.put(node.getName(), newNode);
        ...
        for (Iterator i = edges.iterator(); i.hasNext(); ) {
            ...
            src = (BBNNode) nodeTable.get(src.getName());
            dest = (BBNNode) nodeTable.get(dest.getName());
