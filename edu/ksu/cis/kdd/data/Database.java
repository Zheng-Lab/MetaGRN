...
import edu.ksu.cis.kdd.data.converter.arff.*;
import edu.ksu.cis.kdd.data.converter.xml.*;
import edu.ksu.cis.kdd.data.converter.csf.*;
import edu.ksu.cis.kdd.data.converter.dat.*;
import edu.ksu.cis.kdd.data.converter.libb.*;
...
public class Database implements Data {
    ...
    /* public static Database load(String filename, String format) {
     ...
     public static Database load(String filename, String format){
 		...
 		if (t == null){
 			throw new RuntimeException("Error: database object is null");
 ...
 // createTableFromArray method
    public static Database createTableFromArray(int [][] dbnData){
    	Database t = null;
    	ArffParser arr = new ArffParser();
    	t= arr.createTableFromArray(dbnData);
    	if (t == null){
			throw new RuntimeException("Error: database object is null");
    ...
    protected static void assignDefaultType(Attribute attr, int sqlType) {
        switch (sqlType) {
            case Types.INTEGER:
                attr.setType(Attribute.INTEGER); break;
            case Types.DOUBLE:
            case Types.FLOAT:
                attr.setType(Attribute.REAL); break;
            case Types.VARCHAR:
...
//                }
                ...
                while (rs2.next()) {
                    ...
                    ResultSet rs3 = db.remoteStatement.executeQuery(
                              "SELECT COUNT(DISTINCT "+ // $NON-NLS-1$
                    ...
                    if (isMySQL) {
                        ...
                        while (tok.hasMoreTokens()) {
                            ...
                            while (lineTok.hasMoreTokens()) {
                                ...
                                switch (state) {
                                    case 0:
                                        if (token.equals("CONSTRAINT")) state = 1; // $NON-NLS-1$
                                        else if (token.equals("FOREIGN")) state = 3; // $NON-NLS-1$
                                    ...
                                    case 1:
                                        ...
                                        break;
                                    case 2:
                                        if (token.equals("FOREIGN")) state = 3; // $NON-NLS-1$
                                        else state = -1; // INVALID
                                        break;
                                    case 3:
                                        if (token.equals("KEY")) state = 4; // $NON-NLS-1$
                                        else state = -1; // INVALID
                                        break;
                                    case 4:
                                        ...
                                        break;
                                    case 5:
                                        if (token.equals("REFERENCES")) state = 6; // $NON-NLS-1$
                                        else state = -1; // INVALID
                                        break;
                                    case 6:
        ...
        try {
            ...
            for (int i = numTables-1; i >= 0; i--) {
                ...
                for (Iterator j = attrs.iterator(); j.hasNext(); attrIndex++) {
                            ...
                            for (Iterator k = attr.getValues().iterator(); k.hasNext(); ) {
                                ...
                                switch (checkValueType(curValue)) {
                                    case Attribute.INTEGER:
                                         ...
                                         break;
                                    case Attribute.REAL:
                                         curVarType = Attribute.REAL; break;
                                    case Attribute.DISCRETE:
                                         curVarType = Attribute.DISCRETE; break;
                                }
                    ...
                    if (attr.isReferenceKey()) sqlCommand.append(" NOT NULL"); // $NON-NLS-1$
