...
public class GUIUtil {
    ...
    public static KeyStroke translateShortcut(String shortcut) {
        ...
        while (tok.hasMoreTokens())
        {
            ...
            if (s.equals("shift")) { modifier |= InputEvent.SHIFT_MASK; continue; } else // $NON-NLS-1$
            if (s.equals("control") || s.equals("ctrl")) { modifier |= InputEvent.CTRL_MASK; continue; } else // $NON-NLS-1$ // $NON-NLS-2$
            if (s.equals("meta")) { modifier |= InputEvent.META_MASK; continue; } else // $NON-NLS-1$
            if (s.equals("alt")) { modifier |= InputEvent.ALT_MASK; continue; } else // $NON-NLS-1$
            if (s.equals("button1")) { modifier |= InputEvent.BUTTON1_MASK; continue; } else // $NON-NLS-1$
            if (s.equals("button2")) { modifier |= InputEvent.BUTTON2_MASK; continue; } else // $NON-NLS-1$
            if (s.equals("button3")) { modifier |= InputEvent.BUTTON3_MASK; continue; } else // $NON-NLS-1$
            if (s.equals("released")) { released = true; continue; } else // $NON-NLS-1$
            if (s.equals("pressed")) continue; else // $NON-NLS-1$
            if (s.equals("typed")) { typed = true; continue; } // $NON-NLS-1$

            if (typed)
            {
                if (s.equals("\\\"")) return KeyStroke.getKeyStroke('"'); // $NON-NLS-1$ // $NON-NLS-2$
                else if (s.length() != 1) return null;
                else return KeyStroke.getKeyStroke(s.charAt(0));
            }
