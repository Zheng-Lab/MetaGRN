...
public class StringUtil {
    ...
    private StringUtil() {} // make it non-instantiable
    ...
    public static String unescapeString(String str) {
        ...
        while (tokenizer.hasMoreTokens()) {
            ...
            if (hasEscape) {
                ...
                switch(escapeCode) {
                    case 'n':  token = "\n" + token.substring(1); break;  // $NON-NLS-1$
                    case 'r':  token = "\r" + token.substring(1); break;  // $NON-NLS-1$
                    case 't':  token = "\t" + token.substring(1); break;  // $NON-NLS-1$
                    case '"':  token = "\"" + token.substring(1); break;  // $NON-NLS-1$
                    case '\'': token = "\'" + token.substring(1); break;  // $NON-NLS-1$
                    case '\\': token = "\\" + token.substring(1); break;  // $NON-NLS-1$
                    case 'b':  token = "\b" + token.substring(1); break;  // $NON-NLS-1$
                    case 'f':  token = "\f" + token.substring(1); break;  // $NON-NLS-1$
